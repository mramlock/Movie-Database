...
class BST:
    ...
    def __contains__(self, key):
        ...
        if self._root is None:
            ...
            contain = self._contains_aux(key, self._root)
        return contain
    ...
    def _contains_aux(self, key, node):
        ...
        if node is not None:
            if node._value == key:
                ...
                contain = self._contains_aux(key, node._right)
                if contain == False:
                    self._contains_aux(key, node._left)
        return contain
    ...
    def __eq__aux(self, self_node, target_node):
        equals = False
        if self_node is None and target_node is None:
            equals = True
        elif (self_node is None and target_node is not None) or (self_node is not None and target_node is None):
            equals = False
        elif self_node._value == target_node._value:
            equals = self.__eq__aux(self_node._right, target_node._right) and self.__eq__aux(
                self_node._left, target_node._left)
        return equals
    ...
    def parent(self, key):
        ...
        a = None
        b = self._root
        while b is not None and b._value != key:
            a = b
            if key > b._value:
                b = b._right
            elif b._value > key:
    ...
    def parent_r(self, key):
        ...
        # your code here
        return self._parent_r_aux(key, self._root, self._root)

    def _parent_r_aux(self, key, a, b):
        value = None
        if a is not None:
            if a._value == key:
                value = b._value
            else:
                value = self._parent_r_aux(key, a._right, a)
                if value is None:
                    value = self._parent_r_aux(key, a._left, a)
        return value
    ...
    def _leaf_count_aux(self, n):
        if n is None:
            i = 0
        elif n._left is None and n._right is None:
            i = 1
        else:
            i = self._leaf_count_aux(n._right) + self._leaf_count_aux(n._left)
        return i
    ...
    def _two_child_count_aux(self, n):

        count = 0
        if n is not None:
            if n._right is not None and n._left is not None:
                count = 1
            count += self._two_child_count_aux(n._right)
            count += self._two_child_count_aux(n._left)
        return count
    ...
    def _one_child_count_aux(self, n):
        count = 0
        if n is not None:
            if n._left is None and n._right is not None:
                count = 1
            elif n._left is not None and n._right is None:
                count = 1
            count += self._one_child_count_aux(n._right)
            count += self._one_child_count_aux(n._left)
        return count
    ...
    def _is_balanced_aux(self, n):

        if n is None:
            balanced = True
        else:
            if n._left is not None and n._right is not None:
                if abs(n._right._height - n._left._height) <= 1:
                    balanced = self._is_balanced_aux(n._right)
                    if balanced:
                        balanced = self._is_balanced_aux(n._left)
                else:
                    balanced = False
            elif n._left is not None and n._left._height > 1:
                balanced = False
            elif n._right is not None and n._right._height > 1:
                balanced = False
            else:
                balanced = True
        return balanced
    ...
    def _inorder_aux(self, self_node):

        a = []
        if self_node is not None:
            a += self._inorder_aux(self_node._left)
            a.append(self_node._value)
            a += self._inorder_aux(self_node._right)
        return a
    ...
    def _preorder_aux(self, a, n):

        if n is not None:
            a.append(n._value)
            self._preorder_aux(a, n._left)
            self._preorder_aux(a, n._right)
        return
    ...
    def _postorder_aux(self, a, n):

        if n is not None:
            self._postorder_aux(a, n._left)
            self._postorder_aux(a, n._right)
            a.append(n._value)
        return
    ...
    def levelorder(self):
        ...
        if self._root is not None:
            a.append(self._root)
            values = self._levelorder_aux(a)
        return values

    def _levelorder_aux(self, a):

        values = []
        if len(a) != 0:
            n = a.pop(0)
            values.append(n._value)
