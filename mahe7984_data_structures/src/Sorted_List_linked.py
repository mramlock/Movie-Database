...
class Sorted_List:
    ...
    def clean(self):
        ...
        while n is not None:
            c = n._next
            while c is not None and c._value == n._value:
                n._next = c._next
                self._count -= 1
                c = c._next
            if c is None:
    ...
    def intersection(self, source1, source2):
        ...
        while s1 is not None and s2 is not None:
            if s1._value > s2._value:
                s2 = s2._next
            elif s2._value > s1._value:
                s1 = s1._next
            else:
                if self._front is None:
                    self._front = _SL_Node(s1._value, None)
                    self._rear = self._front
                    self._count += 1
                elif s1._value > self._rear._value:
                    self._rear._next = _SL_Node(s1._value, None)
    ...
    def union(self, source1, source2):
        ...
        while s1n is not None and s2n is not None:

            if s1n._value < s2n._value:
                value = s1n._value
                s1n = s1n._next
            elif s1n._value > s2n._value:
                value = s2n._value
                s2n = s2n._next
            else:
                value = s1n._value
        ...
        if self._front is None and sn is not None:
            self._front = _SL_Node(sn._value, None)
            self._rear = self._front
            self._count += 1
            sn = sn._next
        while sn is not None:
            if self._rear._value < sn._value:
                self._rear._next = _SL_Node(sn._value, None)
                self._rear = self._rear._next
                self._count += 1
            sn = sn._next
        return
    ...
    def __eq__(self, target):
        ...
        if self._count != target._count:
            ...
            c1 = self._front
            c2 = target._front
            while c1 is not None and c1._value == c2._value:
                c2 = c2._next
                c1 = c1._next
            equals = c1 is None
        return equals
